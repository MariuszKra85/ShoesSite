  ...
  let validators = {
    notBlank: function (value, option) {
      ...
      return value.length > 0
        ? { valid: true }
        : { valid: false, message: "Field cannot be left blank" };
    },
    ...
    correctLength: function (value, option) {
      ...
      return requiredLength === value.length
        ? { valid: true }
        : {
            valid: false,
            message: `Quantity required ${requiredLength} characters`,
          };
    },
    quantityChecked: function (valueGroupFields) {
      if (valueGroupFields.length > 4 || valueGroupFields.length < 2) {
        return { valid: false, message: "Tick between 2 and 4 boxes" };
      } else {
        return {
          valid: true,
        };
          ...
          if (inp.getAttribute("data-group")) {
            if (isgroup) {
              ...
              for (let validationRegulName in validationReguls) {
                ...
                let validationResult = validationFunction(
                  fieldValue,
                  validOption
                );
